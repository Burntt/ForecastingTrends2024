...
# Generating the time-series plot with attention mechanism
...
# Use attention weights from the lower triangular matrix for the last node
for (u, v, wt) in G.edges.data('weight'):
    if u == n_nodes - 1:  # Only the last node (T=now) has edges with weights other than 0
        wt = normalized_lower_triangular_attention_matrix[u, v]
    else:
        wt = 0
...
fig.savefig('./img/pdf/full_attn_time-series.pdf', format='pdf', bbox_inches='tight')
